...
class Logros extends CI_Controller
{
		...
		if (!empty($logros) && !empty($id_logros)) {
			foreach ($id_logros as $logro) {
				if ($this->Teacher_m->insertLogros($logro, $logros_toString)) {
					echo json_encode(array('message' => 'Error al insertar indicadores de desempeño, intentolo más tarde'));
				}else{
					echo json_encode(array('success' => 'Indicadores de desempeño insertados correctamente'));
				}
			}
		} else {
			echo json_encode(array('message' => 'Error, debe de seleccionar un grupo para insertar los indicadores de desempeño'));
		}
	}
	...
	public function getAsignaturas()
	{
		...
		if (!$res = $this->Teacher_m->getAsignaturasId($id_user)) {
			echo json_encode(array('message' => 'Aún no se le han asignado grupos para insertar indicadores de desempeño'));
		} else {
